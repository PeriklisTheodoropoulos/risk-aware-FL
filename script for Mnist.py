...
cvar_alpha_list = [0.3] #[ 1, 0.3, 0.2, 0.1 ]
# Value of gamma parameter 
gamma_list = [ 0.3 ]  #[  0.1 , 0. ]         #[0.3, 0.2] # [1. , 0.5, 0.4 ] #, 0.1 , 0. ]
...
## (Both two lines below are useful only for fast results, if you need to take fast results you reduce the number of data by choosing reduce_to_ratio -> 0., but this is not essential for the algorithm) 
...
dataset = "mnist"
dataset_train = datasets.MNIST(root='datasets/' + dataset, download=True, transform= transforms)
dataset_train.targets = torch.as_tensor(np.array(dataset_train.targets))
dataset_test = datasets.MNIST(root='datasets/' + dataset, train=False, download=True ,transform= transforms )
...
dataset_train_subset_1 = datasets.MNIST(root='datasets/' + dataset, train=True, download=True ,transform= transforms)
dataset_train_subset_1.targets = dataset_train.targets[idx_subset_train_1]
dataset_train_subset_1.data = dataset_train.data[idx_subset_train_1]
...
dataset_train_subset_2 = datasets.MNIST(root='datasets/' + dataset, train=True, download=True ,transform= transforms)
dataset_train_subset_2.targets = dataset_train.targets[idx_subset_train_2]
dataset_train_subset_2.data = dataset_train.data[idx_subset_train_2]
...
dataset_test_subset= datasets.MNIST(root='datasets/' + dataset, train=False, download=True ,transform= transforms  )
...
users_dataloaders = []
for user in range(K):
    x_train_tensor = data_users[user].clone().detach().float() 
...
def train_mnist(user, global_model , device , global_t, H , train_loader):
    # For each user 
...
path = "official"
experiment_setup = f"{path}_[{len(label_subset_1 + label_subset_2)}]_classes_users_[{K}]/cvar_[{cvar_alpha}]/_gamma_[{gamma}]_learning_rate_[{learning_rate}]_learning_rate_t_[{learning_rate_t}]_local_epochs_[{H}]_batch_size_[{batch_size}]_percentage_[{percentage}]_fast_example_reduce_to_ratio_[{reduce_to_ratio}]_2_classes"    
...
# Call the model
global_model = MLP(n_class = number_classes ,  hidden_size = hidden_size)
...
for epoch in trange(1,num_epochs+1):
    ...
    results = ray.get( [ train_mnist.remote( user, global_model , device , global_t, H , users_dataloaders[user] ) for user in range(K) ] )

    user_weights = [result[0] for result in results]
    t_list = [result[1] for result in results]
    train_loss_list = [result[2] for result in results]
    ...
    for i in range(K):
        writer.add_scalar(f"Training loss vs global epoch/user_{i}", train_loss_list[i], epoch)
    ###########################################################################
    ###########################################################################
...
print(f" Sampling counts = {sampling_counts}, a_i: {a_i} ") 
ray.shutdown()
######################################################################################################################################################
######################################################################################################################################################
